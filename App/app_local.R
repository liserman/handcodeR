# ============================================================================ #
# handcodeR Web App — local / RStudio build                                    #
# ---------------------------------------------------------------------------- #
# Self-contained. Sequential phases run in the RStudio viewer; the annotated     #
# frame is returned to the workspace (no download). Quicksave is enabled. Zero   #
# package changes; stopApp kept. Use from RStudio:                               #
#   handcode_result <- source("app_local.R")$value   # or just: source(...)     #
# ============================================================================ #
...
# Local single-user build is trusted: no upload cap, no row cap.
MAX_ROWS <- Inf
...
# ============================================================================ #
# Orchestration — sequential phases in the RStudio viewer                      #
# ============================================================================ #

run_local <- function() {
  ns <- asNamespace("handcodeR")
  viewer <- shiny::dialogViewer("handcodeR", width = 1200, height = 850)

  config <- runGadget(shinyApp(configurator_ui, configurator_server), viewer = viewer)

  # Local build enables quicksave, pointed at the working directory.
  save_loc <- tryCatch(ns$.quicksave_setup(getwd(), "handcode_data"), error = function(e) NULL)
  app_data <- build_app_data(config, save_loc = save_loc)
  ...
  # Own runGadget -> stopApp(annotated) return is caught here and handed back.
  annotated <- runGadget(shinyApp(build_ui(app_data), build_server(app_data)), viewer = viewer)

  message("\nYour annotated data was returned to the R workspace.")
  print(utils::head(annotated))
  annotated
}

# RStudio: source() runs it and leaves the result in the workspace.
# HANDCODER_DEFS_ONLY is the test/inspection hatch.
if (interactive() && !nzchar(Sys.getenv("HANDCODER_DEFS_ONLY"))) {
  handcode_result <- run_local()
}
