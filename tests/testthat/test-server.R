...
test_that("categorial server: initial counter equals start_val", {
  ...
  server_fn <- handcodeR:::.categorial_server(app_data)
...
test_that("binary server: initial counter equals start_val", {
  ...
  server_fn <- handcodeR:::.binary_server(app_data)
  shiny::testServer(server_fn, {
    expect_equal(values$counter, 1L)
  })
})
...
test_that("comparison server: initial counter equals start_val", {
  ...
  server_fn <- handcodeR:::.comparison_server(app_data)
  shiny::testServer(server_fn, {
    expect_equal(values$counter, 1L)
  })
})
...
# ---------------------------------------------------------------------------- #
# Quicksave, save-and-exit, and unexpected-close flows via testServer().       #
# ============================================================================ #

test_that("save handler: unexpected close returns annotated data to the R session", {
  skip_on_cran()
  # No quicksave dir: nothing is written to disk, but the work must still come back to the R session.
  captured <- list()
...
test_that("binary_comparison server: initial counter equals start_val", {
  ...
  server_fn <- handcodeR:::.binary_comparison_server(app_data)
  shiny::testServer(server_fn, {
    expect_equal(values$counter, 1L)
  })
})
...
test_that("binary_comparison server: quicksave writes file without comparison context columns", {
  skip_on_cran()
  tmp_dir <- tempfile()
  dir.create(tmp_dir)
  on.exit(unlink(tmp_dir, recursive = TRUE))
  ...
  saved_file <- list.files(tmp_dir, pattern = "^bc_test_quicksave_[0-9]+\\.RData$", full.names = TRUE)
  expect_equal(length(saved_file), 1L)
  env <- new.env()
  load(saved_file, envir = env)
  saved_df <- env[["bc_test"]]
