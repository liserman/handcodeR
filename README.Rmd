...
`quicksave` defaults to `NULL` (no button, nothing written to disk). If the directory does not exist, the call stops with an error rather than creating it. To resume from a quicksave file later, load it manually and pass it back to `handcode()`:
